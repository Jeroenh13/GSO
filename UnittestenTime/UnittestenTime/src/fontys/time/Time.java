...
public class Time implements ITime {
    ...
    public Time(int y, int m, int d, int h, int min) {
        ...
        if (min < 0 || min > 59) { //m had to be changed to min
        ...
        try
        {
            ...
            valid.set(y,m-1,d,h,min); //months go from 0 to 11, that's why month is -1
            valid.getTime();
        }
        catch(Exception e){throw new IllegalArgumentException("date is not valid");}
    ...
    public DayInWeek getDayInWeek() {
        int day_of_week = gc.get(GregorianCalendar.DAY_OF_WEEK_IN_MONTH); // need day of week in month instad of day of week
        switch (day_of_week) {
    ...
    @Override
    public int getMonth() {
        return gc.get(GregorianCalendar.MONTH)+1; //getmonth doesn't need a +1
    }
    ...
    public int difference(ITime time) {
        Time t = (Time) time;
        return (int) (-(this.gc.getTimeInMillis() - t.gc.getTimeInMillis()) / 60000); //there was a zero too much
        //there had to be a minus before (this, so you don't get a minvalue
    }
}
